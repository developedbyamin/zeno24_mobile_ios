...
struct CirclePillPickerOverlay: View {
    ...
    var body: some View {
        ...
        .onAppear {
            withAnimation(.easeOut(duration: 0.22)) { didAppear = true }
        }
    }
    ...
    private var card: some View {
        ...
        let listHeight = CircleFlowMetrics.rowHeight * CGFloat(visibleCount)
            ...
            ScrollView(.vertical, showsIndicators: isScrollable) {
                VStack(spacing: 0) {
                    ForEach(Array(circles.enumerated()), id: \.element.id) { idx, circle in
                        row(for: circle)
                        if idx < circles.count - 1 { rowDivider }
                    }
                }
            }
    ...
    private func row(for circle: CircleModel) -> some View {
        ...
        .background(isHighlighted ? AppColors.surfaceMuted : Color.clear)
        .overlay(
            Shimmer()
    ...
    private var rowDivider: some View {
        HStack {
            Spacer()
            AppColors.surfaceMuted.frame(height: CircleFlowMetrics.rowDividerHeight)
        }
        .frame(width: 276, height: CircleFlowMetrics.rowDividerHeight)
    }
